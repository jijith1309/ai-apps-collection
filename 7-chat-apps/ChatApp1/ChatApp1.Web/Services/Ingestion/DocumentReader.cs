...
namespace ChatApp1.Web.Services.Ingestion;
...
internal sealed class DocumentReader(DirectoryInfo rootDirectory) : IngestionDocumentReader
{
    ...
    public override Task<IngestionDocument> ReadAsync(Stream source, string identifier, string mediaType, CancellationToken cancellationToken = default)
        => mediaType switch
        {
            ...
            "application/msword" => _pdfReader.ReadAsync(source, identifier, mediaType, cancellationToken),
            "application/vnd.openxmlformats-officedocument.wordprocessingml.document" => _pdfReader.ReadAsync(source, identifier, mediaType, cancellationToken),
